...
- is it conceivable that Satoshi simply lost his seedphrase?
  - if I were Satoshi, I would mine a lot of bitcoin for public appearance (to never use it) and then mine extra smaller wallets in secret. Although if all PoW-mining reward addresses are generated from same seed, they all could get lost together. Or, more realisticially, using same (or combination of weak) entropy source naively could've compromised all "my" wallets.
