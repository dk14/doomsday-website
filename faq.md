...
- stance on personal health and ergonomics?
  - "Economics and ecology" in privacy section explains negative impact on privacy. Privacy leads to health, e.g. sleep cycles are affected by imprper redirected foraging, and consequent breakage of privacy. Personall efficiency is directly affected. Machines as one of the source of dreams partially compensate that, but inefficient acquissition (e.g. sillicon mining efficiency depends on personal performance) and use of computational resources limit machines. Bad ergonomics (see warriors README) limit contact. Thus HCI and abstract machine research in warriors. FUn & Profit meant to draw from that.
      - Plus, TRNGs have intimate relationship with novelty, truly secure random number must be not only private, but also novel (unique and hard to compute from other publicly and privately known numbers). Faking that novelty, over-estimating novelty leads to overstimation of entropy available - fake dreams (and fake solutions in wake-up life). "Fake entropy" is most critical issue, that is why whole domsday ecosystem is grounded in solving it, with competitive entropy Explorer Network meant to provide.
